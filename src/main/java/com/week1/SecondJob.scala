...
//      System.setProperty("hadoop.home.dir", "D:\\hadoop-2.5.2")
      ...
      val document = sc.textFile("hdfs://mycluster/user/hive/warehouse/getdatas.db/s_keys/result").map(_.split(" ").toSeq)
      val hashingTF = new HashingTF()
      val tf= hashingTF.transform(document).cache()
    document.foreach(v=>{
      println(v,hashingTF.transform(v))
    })
